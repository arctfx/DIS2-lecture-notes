...
\begin{document}
	...
	\boxed{\varepsilon>0} $\Rightarrow \exists n_0\in \mathbb{N} \spc\forall n \geq n_0 \spc\forall p \in \mathbb{N} \spc\forall x \in D : |f_n(x) - f_{n+p}(x)|<\frac{\varepsilon}{2}$ (След граничен преход $p \rightarrow +\infty$) $\underline{\forall n \geq n_0 \spc\forall x \in D}: |f_n(x)-f(x)|\leq \frac{\varepsilon}{2}<\varepsilon.$ $\square$\\
	%END PROOF
	$\spc$\\
	$\spc$\\
